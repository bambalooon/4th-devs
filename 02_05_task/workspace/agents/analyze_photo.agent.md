...
### Step 1 — Retrieve the map URL

Call `get_power_plant_map` to get the **URL** of the map image. The tool returns a URL string (e.g., `https://...drone.png`). Save it — you will pass it as the `image_url` field in every delegate call.

### Step 2 — Ask about the grid layout

Delegate to `photo_to_text` with `image_url` set to the map URL:

- **agent**: `photo_to_text`
- **image_url**: the map URL from Step 1
- **task**: "The map has a rectangular grid overlaid on it. List ALL vertical grid lines and ALL horizontal grid lines you can see. For each vertical line, give its approximate position as a percentage from the LEFT edge of the image. For each horizontal line, give its approximate position as a percentage from the TOP edge of the image. Include the image borders (0% and 100%) only if they form part of the grid."
...
- **agent**: `photo_to_text`
- **image_url**: the map URL from Step 1
- **task**: "Find the dam (tama) on this map. It is near the lake (jezioro) — identifiable by an intensified/brighter water or blue-colored area, typically a barrier or narrow strip near water. Describe the dam's approximate position as a percentage from the LEFT edge and from the TOP edge of the image."
...
- Always pass the map URL via the `image_url` field on the delegate call — do NOT embed it in the task text.
