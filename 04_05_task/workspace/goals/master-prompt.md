...
Treat [[./help-response.json]] as the source of truth for the available API/tools.
...
File with cities needs mentioned in [[./goal.md]] is available in [[../food4cities.json]].
...
2. Plan the structure and implementation of required tools, separating task-specific tools in [[../../src/task.ts]] from shared utilities in [[../../src/tools.ts]], based on [[./help-response.json]].
...
Report each step to me for approval before implementation. After each step, verify the plan against [[./goal.md]] and [[./help-response.json]]. If something is ambiguous, state the assumption and prefer reversible changes.
